...
def strategy(data, symbol):
    ...
    # This is the buy condition. If we have no positions for the symbol in question, and we have enough cash to buy the
    # asset, we will buy the asset.
    if positions == 0 and cash_to_use > 0:

        # buy the asset
        if (data['rsi'].iloc[-1] < 20) and (data['close'].iloc[-1] < data['ema'].iloc[-1]) and (
                data['close'].iloc[-2] < data['close'].iloc[-1]):
            # place the order
...
if __name__ == '__main__':
    while True:
        symbol = 'BTC/USD'
        lookback = 1
        data = stream_data(symbol, lookback)
